...
#region Private Methods
        ...
        private XmpNode ParseRDF(XElement rdf_node, TagLib.File file)
        {
            ...
            foreach (XElement node in rdf_node.Elements())
            {
            ...
            else
            {
                ...
                else
                {
                    foreach (XAttribute attr in node.Attributes())
                    {
                        ...
                        {
                            // Neither Literal, Collection or anything else is allowed
                            throw new CorruptFileException(String.Format("This is not allowed in XMP! Bad XMP: {0}", node.Parent.Name.LocalName));
                        }
                    }
                }
            }
...
#region Public Methods
		...
		static void EnsureNamespacePrefix (string ns)
		{
			if (!NamespacePrefixes.ContainsKey (ns)) {
				...
				Debug.WriteLine (string.Format("TAGLIB# DEBUG: Added {0} prefix for {1} namespace (XMP)", NamespacePrefixes [ns], ns));
			}
		}
