...
#region Internal Methods
		...
		internal void Dump (string prefix) {
			Debug.WriteLine (string.Format("{0}{1}{2} ({4}) = \"{3}\"", prefix, Namespace, Name, Value, Type));
			if (qualifiers != null) {
				Debug.WriteLine (string.Format("{0}Qualifiers:", prefix));
			...
			if (children != null) {
				Debug.WriteLine (string.Format("{0}Children:", prefix));
